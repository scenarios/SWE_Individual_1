...
//			X[k].i = sin(-2 * m_pi * k / length_current) * x[1];
		...
		*/
		...
		for (int k = 0; k < length_current / 2; k++){
			X[k].r = temp1[k].r + cos(-2 * m_pi * k / length_current) * temp2[k].r + sin(2 * m_pi * k / length_current) * temp2[k].i;
			...
			X[k].i = temp1[k].i + cos(-2 * m_pi * k / length_current) * temp2[k].i - sin(2 * m_pi * k / length_current) * temp2[k].r;
...
//			x[n].i = 1.0 / length_total * (X[0].i + sin(2 * m_pi * n / length_current) * x[1].r + cos(2 * m_pi * n / length_current) * X[1].i);
	...
	*/
		...
		for (int n = 0; n < length_current / 2; n++){
			x[n].r = temp1[n].r + cos(2 * m_pi * n / length_current) * temp2[n].r - sin(2 * m_pi * n / length_current) * temp2[n].i;
			x[n + length_current / 2].r = temp1[n].r + cos(2 * m_pi * (n + length_current / 2) / length_current) * temp2[n].r - sin(2 * m_pi * (n + length_current / 2) / length_current) * temp2[n].i;
			x[n].i = temp1[n].i + cos(2 * m_pi * n / length_current) * temp2[n].i + sin(2 * m_pi * n / length_current) * temp2[n].r;
			x[n + length_current / 2].i = temp1[n].i + cos(2 * m_pi * (n + length_current / 2) / length_current) * temp2[n].i + sin(2 * m_pi * (n + length_current / 2) / length_current) * temp2[n].r;
		}
		delete[] temp1;
		delete[] temp2;
		delete[] X1;
